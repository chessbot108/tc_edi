...
int main() {
	...
	for (int last_half = 1, tot_pairs = 0; last_half <= min(maxa, maxn - first_half); last_half++)
	{
		...
		// bin search on max val of x
		int l = 0, r = maxn;
		while (l != r) {
			long long x = (l + r + 1) / 2;
			long long n = x * log(x) / log(2) * first_half + x * last_half;
		...
		worst_case = max(worst_case, { {tot_pairs * l * log(l) / log(2), last_half}, {tot_pairs,l } });
	}
